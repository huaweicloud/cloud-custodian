...
class DatabaseVersionFilter(Filter):
    ...
    def process(self, resources, event=None):
        ...
        for resource in resources:
            ...
            instance_version_to_compare = '.'.join(version_parts[:3]) if len(version_parts) >= 3 else complete_version

            # 检查主版本是否有对应的最新小版本
            if major_version in latest_versions:
                latest_version = latest_versions[major_version]
                latest_version_parts = latest_version.split('.')
                latest_version_to_compare = '.'.join(latest_version_parts[:3]) if len(latest_version_parts) >= 3 else latest_version

                # 比较版本时只比较前三部分，忽略后面的构建号
                if self._compare_versions(instance_version_to_compare, latest_version_to_compare) < 0:
                    self.log.debug(
                        f"实例 {resource['name']} "
