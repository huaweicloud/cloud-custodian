...
class BmsInstanceUserData(ValueFilter):
    ...
    Note: It is recommended to use regular expressions, as Custodian uses re.match() and the user data may span multiple lines.
    ...
    def process(self, resources, event=None):
        ...
        with self.executor_factory(max_workers=3) as w:
            ...
            for f in as_completed(futures):
                if f.exception():
                    self.log.error(
                        "Error occurred while processing bare metal server user data %s", f.exception()
                    )
    ...
    def process(self, resources, event=None):
        ...
        with self.executor_factory(max_workers=3) as w:
            ...
            for f in as_completed(futures):
                if f.exception():
                    self.log.error(
                        "Error occurred while processing bare metal server volume information %s", f.exception()
                    )
                results.extend(f.result())
        return results
