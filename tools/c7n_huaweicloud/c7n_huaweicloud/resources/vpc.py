...
class Port(QueryResourceManager):
    ...
    def get_api_resources(self, query=None):
        ...
        client = session.client("vpc")
        resourceTagDict = {}
        offset, limit = 0, 1000
        while True:
            try:
                requestTag = ListPortsByTagsRequest()
                requestTag.limit = limit
                requestTag.offset = offset
                requestTag.body = ListPortsByTagsRequestBody()
                responseTag = client.list_ports_by_tags(requestTag)
                ...
                log.error(f"Failed to query port tags, "
...
class SecurityGroupUnAttached(Filter):
    ...
    def process(self, resources, event=None):
        ...
        try:
            ...
            log.debug("[filters] - unattached - The resource: vpc-security-group "
                      "filter unattached security groups succeed")
        except exceptions.ClientRequestException as ex:
    ...
    def process(self, resources, event=None):
        ...
        if self.data.get('DefaultSG', None) is not None:
            ...
            try:
                ...
                log.debug(f"[filters] - {self.direction} - The resource: vpc-security-group-rule "
                          f"filter rules in default security group succeed")
            except exceptions.ClientRequestException as ex:
        ...
        for direction, mode in [('ingress', i_mode), ('egress', e_mode)]:
            ...
            # remove all rules in the security group of the matched rules
                ...
                try:
                    ...
                    log.debug(f"[actions] - remove-rules - The resource: vpc-security-group-rule "
                ...
                for f in mode:
                    try:
                        ...
                        log.error(f"[actions] - set-rules - The resource: vpc-security-group-rule "
                                  f"remove specific rules failed, "
                                  f"cause: query rules failed, "
                                  f"error_code[{ex.error_code}], error_msg[{ex.error_msg}]")
                        raise ex
...
class SecurityGroupRuleAllowRiskPort(Filter):
                ...
                if rule_ip and rule_ip != '0.0.0.0/0':
                    ...
                    try:
                        ...
                        log.debug(f"[filters]-[rule-allow-risk-ports]-"
                                  ...
                                  f"query address group [{rule_ag_id}] succeed")
                    except exceptions.ClientRequestException as ex:
    ...
    def get_file_content(self, obs_url):
        ...
        try:
            ...
            if resp.status < 300:
                log.debug(f"[filters]-[rule-allow-risk-ports]-"
                          f"The resource:[vpc-security-group-rule] "
                          f"get obs object with obs_url:[{obs_url}] succeed")
    ...
    def get_deny_rules(self, sg_id, direction):
        ...
        try:
            ...
            log.debug(f"[filters]-[rule-allow-risk-ports]-"
                        f"The resource:[vpc-security-group-rule] "
                        f"query deny rules in security group [{sg_id}] succeed")
        except exceptions.ClientRequestException as ex:
            log.warning(f"[filters]-[rule-allow-risk-ports]-"
                        f"The resource:[vpc-security-group-rule] "
                        f"filter rules allowing high risk ports failed, "
    ...
    def _is_missing_route(self, vpc_id, peering_id):
        ...
        try:
            ...
            for rtb_id in rtb_ids:
                ...
                if is_route_exist:
                    break
            log.debug(f"[filters]-[missing-route]-The resource:[vpc-peering] "
                      f"check whether peering[{peering_id}] is missing route succeed")
        except exceptions.ClientRequestException as ex:
            log.error(f"[filters]-[missing-route]-The resource:[vpc-peering] "
                      f"check whether peering[{peering_id}] is missing route failed, "
                      f"cause: query peering routes failed, "
                      f"error_code[{ex.error_code}], error_msg[{ex.error_msg}]")
            raise ex
