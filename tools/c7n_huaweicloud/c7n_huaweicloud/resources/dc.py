...
from c7n_huaweicloud.actions.tms import CreateResourceTagDelayedAction
...
class DCTag(HuaweiCloudBaseAction):
    ...
    def perform_action(self, resource):
        ...
        try:
            ...
            log.info(
                f"Added/updated tag for direct connect {resource_name} ({resource_id}): {key}={value}"
            )
        except exceptions.ClientRequestException as e:
            # Handle API request exceptions
            log.error(
                f"Unable to add/update tag {key} for direct connect {resource_name} ({resource_id}): "
                f"{e.error_msg} (status code: {e.status_code})"
            )
        except Exception as e:
            # Handle other potential exceptions
            log.error(
                f"Unable to add/update tag {key} for direct connect {resource_name} ({resource_id}): {str(e)}"
            )
...
class DCRemoveTag(HuaweiCloudBaseAction):
    ...
    def perform_action(self, resource):
        ...
        if not keys_that_exist:
            log.debug(
                f"Direct connect {resource_name} ({resource_id}) does not have tags to remove: {tags_to_remove}"
            )
            return None
        ...
        for key in keys_that_exist:
            try:
                ...
                log.info(
                    f"Removed tag from direct connect {resource_name} ({resource_id}): {key}"
                )
            except exceptions.ClientRequestException as e:
                # Handle API request exceptions
                log.error(
                    f"Unable to remove tag {key} from direct connect {resource_name} ({resource_id}): "
                    f"{e.error_msg} (status code: {e.status_code})"
                )
            except Exception as e:
                # Handle other potential exceptions
                log.error(
                    f"Unable to remove tag {key} from direct connect {resource_name} ({resource_id}): {str(e)}"
                )
