...
class DeleteResourceTagAction(HuaweiCloudBaseAction):
    ...
    def process_resource_set(self, client, resource_batch, tag_batch, project_id):
        request_body = ReqDeleteTag(project_id=project_id, resources=resource_batch, tags=tag_batch)
        request = DeleteResourceTagRequest(body=request_body)
        client.delete_resource_tag(request=request)
        self.log.info("Successfully remove tag %s resources with %s tags", len(resource_batch), len(tag_batch))
