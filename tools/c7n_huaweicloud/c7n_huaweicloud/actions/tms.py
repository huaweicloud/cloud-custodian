    ...
    def process(self, resources):
        ...
        for resource_batch in chunks(resources, RESOURCE_MAX_SIZE):
            try:
                ...
                for failed_resource_res in resource_batch:
                    self.log.warning(
                        f"[actions]-remove-tag The resource "
        ...
        for success_resource_res in resources:
            self.log.info(
                f"[actions]-reomve-tag The resource "
...
class RenameResourceTagAction(HuaweiCloudBaseAction):
    ...
    def process(self, resources):
        ...
        for success_resource_res in self.resources:
            self.log.info(
                f"[actions]-rename-tag The resource "
    ...
    def process_resource(self, resource, old_key, new_key, value):
        try:
            ...
            if not value:
                self.log.error("No value of key %s in resource %s", old_key, resource["id"])
            ...
            if len(delete_response.failed_resources) > 0:
                self.log.warning(f"[actions]-rename-tag The resource "
                ...
                self.log.debug(f"[actions]-rename-tag The resource "
                               f"with id:[{resource['id']} "
            ...
            self.log.warning(
                f"[actions]-rename-tag The resource "
                f"with id:[{resource['resource_id']}] rename tag is failed. "
                f"cause: : {ex.error_msg}, "
                f"status: {ex.status_code}, "
                f"requestId: {ex.request_id}")
            self.handle_exception(failed_resources=[resource], resources=self.resources)
...
class NormalizeResourceTagAction(HuaweiCloudBaseAction):
    ...
    def process(self, resources):
        ...
        for success_resource_res in self.resources:
            self.log.info(
                f"[actions]-normalize-tag The resource "
    ...
    def process_resource(self, resource):
        try:
            ...
            if not self.old_value and not old_value:
                self.log.error("No value of key %s in resource %s", self.key, resource["id"])
                self.handle_exception(failed_resources=[resource], resources=self.resources)
                return
            ...
            new_value = self.get_new_value(old_value, self.action, self.old_sub_str,
                                                self.new_sub_str)
            if not new_value:
                self.log.error("Can not get new value of key %s in resource %s", self.key,
                                   resource["id"])
                self.handle_exception(failed_resources=[resource], resources=self.resources)
                return
            ...
            delete_response = self.tms_client.delete_resource_tag(request=request)
            if len(delete_response.failed_resources) > 0:
                self.log.warning(f"[actions]-normalize-tag The resource "
                                 f"with id:[{resource['id']} "
                                 f"delete tag:{old_tags} is failed. "
                                 f"cause: {delete_response.failed_resources[0].error_msg}")
                self.handle_exception(failed_resources=[resource], resources=self.resources)
                ...
                self.log.debug(f"[actions]-normalize-tag The resource "
                               f"with id:[{resource['id']} "
                               f"delete tag:{old_tags} is success. ")
            ...
            self.log.warning(
                f"[actions]-normalize-tag The resource "
                f"with id:[{resource['resource_id']}] normalize tag is failed. "
                f"cause: : {ex.error_msg}, "
                f"status: {ex.status_code}, "
                f"requestId: {ex.request_id}")
            self.handle_exception(failed_resources=[resource], resources=self.resources)
...
class TrimResourceTagAction(HuaweiCloudBaseAction):
    ...
    def process(self, resources):
        ...
        for success_resource_res in self.resources:
            self.log.info(
                f"[actions]-tag-trim The resource "
    ...
    def process_resource(self, resource, space, preserve):
        try:
            ...
            delete_response = self.tms_client.delete_resource_tag(request=request)
            if len(delete_response.failed_resources) > 0:
                self.log.warning(f"[actions]-tag-trim The resource "
                                 f"with id:[{resource['id']} "
                                 f"delete tag:{old_tags} is failed. "
                                 f"cause: {delete_response.failed_resources[0].error_msg}")
                self.handle_exception(failed_resources=[resource], resources=self.resources)
                ...
                self.log.debug(f"[actions]-tag-trim The resource "
                               f"with id:[{resource['id']} "
                               f"delete tag:{old_tags} is success. ")
        except exceptions.ClientRequestException as ex:
            self.log.warning(
                f"[actions]-tag-trim The resource "
                f"with id:[{resource['resource_id']}] tag trim is failed. "
                f"cause: : {ex.error_msg}, "
                f"status: {ex.status_code}, "
                f"requestId: {ex.request_id}")
            self.handle_exception(failed_resources=[resource], resources=self.resources)
    ...
    def get_delete_keys(self, tags, space, preserve):
        if len(tags) > MAX_TAGS_SIZE:
            self.log.error("Can not perform tag-trim when tags more than %d, please reduce to %d",
    ...
    def process(self, resources):
        ...
        for resource_batch in chunks(resources, RESOURCE_MAX_SIZE):
            try:
                ...
                for failed_resource_res in resource_batch:
                    self.log.warning(
                        f"[actions]-mark-for-op The resource "
                        f"with id:[{failed_resource_res['resource_id']}] create tag is failed. "
                        f"cause: : {ex.error_msg}, "
                        f"status: {ex.status_code}, "
                        f"requestId: {ex.request_id}")
