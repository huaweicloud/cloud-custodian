...
class InstanceStartTest(BaseTest):
    ...
    def test_fetch_job_status(self):
        ...
        p = self.load_policy({
            ...
            'resource': 'huaweicloud.ecs',
            'actions': [{"type": "fetch-job-status", "job_id": "ff8080829585af7f0195c1e776d9475c"}]
        }, session_factory=factory)
        resources = p.run()
        self.assertEqual(len(resources), 1)
    ...
    def test_instance_resize(self):
        ...
        p = self.load_policy({
            ...
            'resource': 'huaweicloud.ecs',
            'filters': [{"type": "value","key":"id","value":"bac642b0-a9ca-4a13-b6b9-9e41b35905b6"}],
            'actions': [{"type": "instance-resize", "flavor_ref": "x1.1u.4g", "mode": "withStopServer"}]
        }, session_factory=factory)
        resources = p.run()
        self.assertEqual(len(resources), 1)
        self.assertEqual(resources[0]['id'], 'bac642b0-a9ca-4a13-b6b9-9e41b35905b6')
...
#----------------------------------------Filter Test------------------------------------------------#
    ...
    def test_instance_attribute(self):
        ...
        p = self.load_policy({
            ...
            'resource': 'huaweicloud.ecs',
            'filters': [{"type": "instance-attribute","attribute":"OS-EXT-SRV-ATTR:user_data",
                         "key":"Value","op":"regex","value":"(?smi).*user="}],
        }, session_factory=factory)
        resources = p.run()
