...
class VpcEndpointTest(BaseTest):
    ...
    @patch('huaweicloudsdksmn.v2.SmnClient.publish_message')
    def test_vpcep_action_send_msg_detail(self, mock_publish_message):
        """Test message sending action implementation details"""
        # Mock a successful send response
        mock_response = MagicMock()
        mock_response.message_id = 'test-message-id'
        mock_publish_message.return_value = mock_response
        ...
        factory = self.replay_flight_data('vpcep_action_send_msg_detail')
        p = self.load_policy({
        ...
        # Test action class directly
        action = p.resource_manager.actions[0]
        result = action.perform_action(test_resource)

        # Verify results
        self.assertEqual(len(result), 1)  # Should have one result
        self.assertEqual(result[0]['status'], 'success')
        self.assertEqual(result[0]['message_id'], 'test-message-id')
        ...
        self.assertIn('com.huaweicloud.service.test', request.body.message)
        self.assertIn('vpc-12345678', request.body.message)
