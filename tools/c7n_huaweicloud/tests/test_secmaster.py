...
class SecmasterTest(BaseTest):
    ...
    def test_secmaster_playbook_name_filter(self):
        ...
        for i, playbook in enumerate(resources):
            self.assertEqual(playbook['name'], expected_names[i], f"第{i+1}个剧本名称应该是{expected_names[i]}")
    ...
    def test_secmaster_workspace_send_msg_when_empty(self):
        ...
        # 根据VCR文件：返回空工作空间列表
        # 虽然设置了send_when_empty=True，但action不创建虚拟资源，而是在process中处理空列表的通知逻辑
        # 最终仍然返回空列表
        self.assertEqual(len(resources), 0, "即使send_when_empty=True，也应该返回空列表，通知逻辑在action内部处理")
    ...
    def test_secmaster_alert_send_msg(self):
        ...
        self.assertEqual(alert['workspace_name'], 'production-workspace', "应该有工作空间名称")
        
        # data_object中的字段
        data_object = alert['data_object']
        self.assertEqual(data_object['title'], '最新高危告警', "告警标题应该是'最新高危告警'")
    ...
    def test_secmaster_playbook_send_msg(self):
        ...
        # 验证第二个剧本
        playbook2 = resources[1]
        self.assertEqual(playbook2['id'], 'playbook-002', "第二个剧本ID应该是playbook-002")
        self.assertEqual(playbook2['name'], '恶意流量监控剧本', "第二个剧本名称应该匹配")
    ...
    def test_secmaster_combined_playbook_actions(self):
        ...
        self.assertEqual(playbook['name'], '高危操作监控剧本', "剧本名称应该包含'高危操作'")
        self.assertFalse(playbook['enabled'], "过滤条件：应该是未启用状态")
        self.assertEqual(playbook['workspace_id'], 'workspace001', "应该有工作空间ID")
        self.assertEqual(playbook['workspace_name'], 'production-workspace', "应该有工作空间名称")
    ...
    def test_secmaster_workspace_based_security_check(self):
        ...
        workspace1 = resources[0]
        self.assertEqual(workspace1['name'], 'production-main', "第一个工作空间名称应该是production-main")
        self.assertEqual(workspace1['id'], '39*************bf', "第一个工作空间ID应该匹配")
        self.assertEqual(workspace1['creator_name'], 'admin', "第一个工作空间创建者应该是admin")
        self.assertEqual(workspace1['description'], '生产环境主工作空间', "第一个工作空间描述应该匹配")
        self.assertFalse(workspace1['is_view'], "第一个工作空间不应该是视图")
        
        # 验证第二个工作空间 - production-backup
        workspace2 = resources[1]
        self.assertEqual(workspace2['name'], 'production-backup', "第二个工作空间名称应该是production-backup")
        self.assertEqual(workspace2['id'], '28*************ae', "第二个工作空间ID应该匹配")
        self.assertEqual(workspace2['creator_name'], 'security_admin', "第二个工作空间创建者应该是security_admin")
        self.assertEqual(workspace2['description'], '生产环境备用工作空间', "第二个工作空间描述应该匹配")
